...
public interface AdoptionRequestRepository extends JpaRepository<AdoptionRequest, UUID> {
    ...
    @Query("SELECT ar FROM AdoptionRequest ar JOIN ar.pet p WHERE p.centerId = :petCenterId AND ar.status = :status")
    List<AdoptionRequest> findByPetCenterIdAndStatus(@Param("petCenterId") UUID petCenterId, @Param("status") AdoptionRequestStatus status);
}